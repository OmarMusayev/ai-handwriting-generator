...
class HandwritingDataset(Dataset):
    ...
    def __init__(self, data_path, split='train', text_req=False, debug=False, max_seq_len=300, data_aug=False):
        ...
        # Ensure sequence consistency by padding strokes
        ...
        # Ensure consistency in text sequences by padding to the maximum length
        ...
        # Pad char sequences
        char_seqs_padded = [
            np.pad(char_seq, (0, max_char_len - len(char_seq)), mode='constant', constant_values=' ')
            for char_seq in char_seqs
        ]
        char_seqs = np.array(char_seqs_padded, dtype='<U1')  # Ensure consistent dtype

        # char Mask
        char_mask = np.zeros((n_total, max_char_len), dtype=np.float32)

        # Input text array
        inp_text = np.array(char_seqs, dtype='<U1')
